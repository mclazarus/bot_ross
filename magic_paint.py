...
def load_magic_library(path):
    ...
    Returns [] (fails open) if the file is missing or malformed. Delegates to
    json_library, the shared load/save/seed logic magic_paint and macros both
    build on."""
    return json_library.load_library(path, label="magic")
...
def seed_magic_library(working_path, default_path):
    ...
    json_library.seed_library(working_path, default_path, label="magic")
