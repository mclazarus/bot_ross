...
`image_size.py` also owns the generation-side sizing for `&paint`/`&hpaint`/`&mpaint`/`&lpaint`/`&xpaint`: `parse_size_flags` strips `--square`/`--landscape`/`--portrait`/`--res WxH` out of the prompt before macro expansion or magic paint ever see it, and `coerce_generation_size` deterministically coerces an arbitrary `--res` into a size the `/v1/images/generations` endpoint accepts (multiple of 16, aspect ratio in `[1/3, 3]`, within a `3840x2160` box). Empirically gpt-image-2's `/v1/images/edits` endpoint honors arbitrary sizes too (despite the docs only listing the three standard sizes + `auto`), so `resolve_edit_size` coerces an explicit `--res` with the **same** `coerce_generation_size` as the generation path rather than snapping it to a standard size. The one place the two regimes still differ is `&remix`'s **default** (no size flag): it snaps the input image's own aspect to a standard size via `edit_size_for_dimensions` (or `auto` when Discord didn't report dimensions). `resolve_generation_size`/`resolve_edit_size` each return `(size, requested)`, and callers post a coercion notice only when `requested` is set and differs from `size` — an orientation preset, an already-valid `--res`, or the no-flag default stays silent. `fetch_image` (generation) and `fetch_image_edit` (edit) both now accept a `size` override, copying `MODEL_CONFIGS[...]["params"]` before applying it so the shared config dict is never mutated. `&dpaint` (dall-e-3 has a different size/model set), `&meme`, and `&release_image` are **not** wired up to any of this; adding dall-e-3 sizing later would mean giving it its own size profile alongside `GEN_*`/`ORIENTATIONS`, since dall-e-3's allowed sizes differ from gpt-image-2's.
